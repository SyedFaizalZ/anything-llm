    ...
    for (const path of additions) {
      ...
      if (metadata.graphMode) {
        try {
          ...
          // Construct absolute path to the raw uploaded file
          // The JSON data has a URL field containing the hotdir path (e.g. file:///.../hotdir/targetFileName.pdf)
          let originalFilename = newDoc.filename;
          if (data.url && typeof data.url === "string") {
              if (data.url.includes("hotdir/")) {
                  originalFilename = data.url.split("hotdir/")[1];
              } else {
                  originalFilename = data.url.split('/').pop();
              }
          } else if (metadata.title) {
              originalFilename = metadata.title;
          }
          const rawFilePath = pathModule.resolve(documentsPath, "raw", originalFilename);
          
          if (!fs.existsSync(rawFilePath)) {
            console.error(`[Graph Mode] Could not find raw file to process: ${rawFilePath}. Make sure the collector copied it properly.`);
            continue;
          }
          ...
          console.log(`[Graph Mode Status] Phase 2: Triggering native RAG-Anything pipeline in the background for ${newDoc.filename}`);
          
          // Execute asynchronously (non-blocking) and stream logs line-by-line
          const graphProcess = spawn('python3', ['-u', 'graph_insert.py'], { cwd: ragAnythingDir, env });

          graphProcess.stdout.on('data', (data) => {
              const output = data.toString().trim();
              if (output) {
                  // Split by newline so multiple prints in one chunk still show properly
                  output.split('\\n').forEach(line => {
                      console.log(`[Graph Worker] ${line}`);
                  });
              }
          });
          ...
          graphProcess.stderr.on('data', (data) => {
              ...
              if (errorOutput) {
                  errorOutput.split('\\n').forEach(line => {
                      console.error(`[Graph Worker Log] ${line}`);
                  });
              }
          });
          ...
          graphProcess.on('close', (code) => {
             if (code !== 0) {
               console.error(`[Graph Mode Background] Knowledge Graph processing failed with exit code: ${code}`);
             } else {
               console.log(`[Graph Mode Status] PHASE 2 COMPLETE: Knowledge Graph is READY for ${newDoc.filename}`);
             }
          });
        } catch (e) {
